...
| genres            | TEXT[]       | NO       | default `'{}'` — แนวเพลง; เติมจาก **metadata ของ YouTube เอง** (yt-dlp categories/tags ตอน resolve stream — fire-and-forget, fail-soft) ตาม [ADR-009](./adr/009-drop-spotify-youtube-only.md) |
...
4. ~~Controlled vocabulary ของ genre?~~ — **ตัดสินใจแล้ว (grilling 2026-09-20): ทางเลือก C** — เก็บ raw tags จาก metadata ของ source (YouTube: yt-dlp categories/tags — normalize เบา ๆ: lowercase/trim) ตาม [ADR-009](./adr/009-drop-spotify-youtube-only.md) + matching แบบ intersection ตรง ๆ ใน MVP; เปิดทางเพิ่ม mapping layer (รวม genre กลุ่มใหญ่) ภายหลังถ้าพบว่า matching แม่นไม่พอ — raw tags ที่เก็บไว้ทำให้ไม่ต้อง re-fetch
